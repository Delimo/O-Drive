...
export async function onRequest(context) {
  ...
  if (path === '/api/search') {
    ...
    const matches = listed.objects.map(o => ({
        ...
        const isNotPlaceholder = f.name !== '' && f.name !== '.folder';
        const isNotHidden = auth.role === 'admin' || !hiddenPaths.some(hp => f.fullKey === hp || f.fullKey.startsWith(hp + '/'));
        return nameMatch && isNotPlaceholder && isNotHidden;
    });
    return jsonResponse({ files: matches.slice(0, 50) });
  }
  ...
  if (path.startsWith('/api/files') && method === 'GET') {
    ...
    const files = (listed.objects || []).map(o => ({ name: o.key.slice(prefix.length), path: '/' + o.key, sizeFormatted: (o.size/1024/1024).toFixed(2)+' MB', rawSize: o.size, time: o.uploaded.getTime(), fullKey: o.key })).filter(f => f.name !== '' && f.name !== '.folder' && (auth.role === 'admin' || !hiddenPaths.includes(f.fullKey)));
