...
const NOTIFY_TABLE = 'CREATE TABLE IF NOT EXISTS notifications (id INTEGER PRIMARY KEY AUTOINCREMENT, event TEXT NOT NULL, message TEXT NOT NULL, path TEXT DEFAULT \'\', read INTEGER NOT NULL DEFAULT 0, created_at INTEGER NOT NULL)';
...
export async function createNotification(env, { event, message, path = '' }) {
  ...
  ).bind(event, message, path, now).run();
}
