...
import { countFileIndexObjectRefs, deleteFileIndexKey, listFileIndexPrefix, updateFileIndexObjectKey, upsertFileIndex } from './file-index.js';
import { resolveExistingObjectLocation, resolveExistingStorageId, resolveStorageIdForPath, storageDelete, storageGet, storageList, storagePut } from './storage.js';
...
export async function copyR2Object(env, sourceKey, targetKey, targetStorageId = '') {
  ...
  await upsertFileIndex(env, targetKey, { ...obj, storageId: destStorageId, objectKey: targetKey });
  return true;
}
...
export async function deletePathEntry(env, path, storageId, objectKey) {
  ...
  await storagePut(env, storageId, hiddenKey, obj.body, { httpMetadata: obj.httpMetadata });
...
export async function copyTree(env, sourceKey, targetKey, move = false) {
  ...
  if (obj) {
    if (!move && sourceStorageId === targetStorageId) {
      ...
      await upsertFileIndex(env, targetKey, { ...obj, storageId: sourceStorageId, objectKey: sourceObjectKey });
    } else {
      await storagePut(env, targetStorageId, targetKey, obj.body, { httpMetadata: obj.httpMetadata });
      await upsertFileIndex(env, targetKey, { ...obj, storageId: targetStorageId, objectKey: targetKey });
    }
    if (move) {
  ...
  await mapWithConcurrency([...subtreeEntries.values()], 6, async item => {
    ...
    const subObj = await storageGet(env, subLocation.storageId, subLocation.objectKey);
    if (subObj) {
      if (!move && subLocation.storageId === targetStorageId) {
        await ensureSourceIndexed(env, logicalKey, subLocation, subObj);
        await upsertFileIndex(env, nextKey, { ...subObj, storageId: subLocation.storageId, objectKey: subLocation.objectKey });
      } else {
        await storagePut(env, targetStorageId, nextKey, subObj.body, { httpMetadata: subObj.httpMetadata });
        await upsertFileIndex(env, nextKey, { ...subObj, storageId: targetStorageId, objectKey: nextKey });
      }
      if (move) {
        await deletePathEntry(env, logicalKey, subLocation.storageId, subLocation.objectKey);
      }
    }
  });
}
