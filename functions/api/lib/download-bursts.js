...
export async function checkDownloadBlocked(env, request, auth) {
  ...
  try {
    const row = await env.D1.prepare(
      "SELECT blocked_until FROM download_bursts WHERE key = ?",
    )
      .bind(key)
      .first();
  ...
  try {
    if (Math.random() < 0.01) {
      await env.D1.prepare(
        "DELETE FROM download_bursts WHERE window_start < ? AND last_alert < ?",
      )
    ...
    const row = await env.D1.prepare(
      "SELECT request_count, window_start, last_alert, blocked_until, sample_paths FROM download_bursts WHERE key = ?",
    )
      .bind(key)
      .first();
    ...
    await env.D1.prepare(
      ...
      .bind(count, lastAlert, blockedUntil, JSON.stringify(samplePaths), key)
      .run();
