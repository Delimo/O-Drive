import { jsonResponse, isHiddenKey, isReservedKey, assertCompleteListing } from './common.js';
import { checkProtectedAccess } from './protected-paths.js';
import { createZipStream } from './zip-stream.js';
import { resolveExistingObjectLocation, storageList, storageGet, storageHead } from './storage.js';
...
async function resolveZipEntries(env, path, hiddenPaths, auth, protectedPaths) {
  ...
  const isFolder = (listed.objects?.length > 0 || listed.delimitedPrefixes?.length > 0);

  const entries = [];

  if (isFolder) {
    const all = await assertCompleteListing(
      await storageList(env, storageId, { prefix }),
      `ZIP folder listing: ${path}`
    );
    ...
    for (const obj of all.objects || []) {
      ...
      if (auth.role !== 'admin') {
        const access = await checkProtectedAccess(null, env, auth, protectedPaths, fullKey);
        if (!access.ok) continue;
      }
      entries.push({
        name: folderName + '/' + relPath,
        size: obj.size,
        getStream: () => storageGet(env, storageId, obj.key).then(r => r?.body || emptyStream()),
      });
    }
  } else {
    if (isHiddenKey(clean, hiddenPaths) && auth.role !== 'admin') return null;
    if (auth.role !== 'admin') {
      const access = await checkProtectedAccess(null, env, auth, protectedPaths, clean);
    ...
    entries.push({
      name: filename,
      size: meta.size,
      getStream: () => storageGet(env, storageId, objectKey).then(r => r?.body || emptyStream()),
    });
  }
...
export async function handleZipDownload(env, request, hiddenPaths, auth, protectedPaths) {
  let body;
  try { body = await request.clone().json(); } catch { return jsonResponse({ success: false, message: 'Invalid JSON body' }, 400); }
  const rawPaths = body.paths;
  if (!Array.isArray(rawPaths) || rawPaths.length === 0) return jsonResponse({ success: false, message: 'paths array required' }, 400);
  ...
  for (const rawPath of rawPaths) {
    const entries = await resolveZipEntries(env, rawPath, hiddenPaths, auth, protectedPaths);
    if (entries) allEntries.push(...entries);
  }

  if (allEntries.length === 0) return jsonResponse({ success: false, message: 'No files to download' }, 404);
