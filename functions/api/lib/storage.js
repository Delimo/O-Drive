...
function parseCapacityBytes(value, fallback = 0) {
  if (typeof value === 'number') return Math.max(0, Math.floor(Number.isFinite(value) ? value : fallback));
...
async function signedS3Request(space, method, key, { body, headers = {}, query = '' } = {}) {
  if (!space.endpoint || !space.bucket || !space.accessKeyId || !space.secretAccessKey) {
    throw new Error(`Storage ${space.name || space.id} is not fully configured`);
  }
  const now = new Date();
  ...
  const sortedHeaders = [...requestHeaders.entries()].map(([k, v]) => [k.toLowerCase(), String(v).trim()]).sort(([a], [b]) => a.localeCompare(b));
  const canonicalHeaders = sortedHeaders.map(([k, v]) => `${k}:${v}\n`).join('');
  const signedHeaders = sortedHeaders.map(([k]) => k).join(';');
  const canonicalRequest = [method, pathname, query.replace(/^\?/, ''), canonicalHeaders, signedHeaders, payloadHash].join('\n');
  const credentialScope = `${dateStamp}/${space.region || 'auto'}/s3/aws4_request`;
  const stringToSign = ['AWS4-HMAC-SHA256', amzDate, credentialScope, await sha256Hex(canonicalRequest)].join('\n');
  const signature = toHex(await hmac(await signingKey(space.secretAccessKey, dateStamp, space.region || 'auto', 's3'), stringToSign));
  requestHeaders.set('authorization', `AWS4-HMAC-SHA256 Credential=${space.accessKeyId}/${credentialScope}, SignedHeaders=${signedHeaders}, Signature=${signature}`);
  requestHeaders.delete('host');
  const res = await fetch(targetUrl, { method, headers: requestHeaders, body });
  if (!res.ok && res.status !== 404) throw new Error(`S3 ${method} failed: HTTP ${res.status}`);
  return res;
}
