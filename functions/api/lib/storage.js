...
function normalizeBinding(item = {}) {
  ...
  const storageId = normalizeStorageId(
    item.storageId || item.storage_id || item.storage,
  );
...
export async function loadStorageConfig(env) {
  ...
  try {
    ...
    const spaces = Array.isArray(raw.spaces)
      ? raw.spaces.map(normalizeStorageItem).filter(Boolean)
      : [];
    ...
    const config = {
      ...
      overflowThresholdPercent: Math.max(
        1,
        Math.min(
          99,
          Number(
            raw.overflowThresholdPercent || fallback.overflowThresholdPercent,
          ),
        ),
      ),
      spaces,
    ...
    await recordSystemWarning(
      env,
      "storage.config",
      err?.message || "Storage config load failed",
    );
    return fallback;
  }
}
...
async function normalizeConfigForSave(config = {}) {
  ...
  return {
    ...
    overflowThresholdPercent: Math.max(
      ...
      Math.min(
        99,
        Number(config.overflowThresholdPercent || DEFAULT_OVERFLOW_THRESHOLD),
      ),
    ),
    spaces,
    bindings,
...
export function publicStorageConfig(config, usage = {}) {
  ...
  return {
    ...
    overflowThresholdPercent: Number(
      config.overflowThresholdPercent || DEFAULT_OVERFLOW_THRESHOLD,
    ),
...
export function storageQuotaForConfig(config, storageId = "r2") {
  ...
  return {
    id: storageId,
    name: space?.name || storageId,
    quotaBytes: Number(space?.quotaBytes || 0),
  };
}
...
export async function resolveStorageIdForPath(env, key) {
  ...
  for (const binding of config.bindings || []) {
    if (
      bindingMatches(binding.path, clean) &&
      binding.path.length > best.path.length
    )
      best = binding;
  }
...
export async function chooseUploadStorage(env, key, incomingBytes = 0) {
  ...
  const r2QuotaBytes = Number(config.r2QuotaBytes || 0);
  const usedPercent = r2QuotaBytes
    ? Math.round((r2UsedBytes / r2QuotaBytes) * 100)
    : 0;
  const projectedPercent = r2QuotaBytes
    ? Math.round(
        ((r2UsedBytes + Number(incomingBytes || 0)) / r2QuotaBytes) * 100,
      )
    : usedPercent;
  ...
  const overflowTarget =
    (config.spaces || []).find(
      (item) => item.enabled !== false && item.overflowTarget,
    ) || (config.spaces || []).find((item) => item.enabled !== false);

  if (
    configuredStorageId === "r2" &&
    config.overflowEnabled &&
    r2QuotaBytes > 0 &&
...
function storageById(config, id) {
  if (!id || id === "r2")
    return { id: "r2", name: "Cloudflare R2", provider: "r2" };
  return (
    (config.spaces || []).find(
...
export async function getStorageAdapter(env, storageId = "r2") {
  ...
  if (space.provider === "r2")
    return { id: "r2", name: "Cloudflare R2", bucket: env.R2, provider: "r2" };
...
export async function storagePut(env, storageId, key, body, options = {}) {
  ...
  const headers = {};
  const contentType =
    options.httpMetadata?.contentType || options.contentType || "";
...
export async function storageCreateMultipartUpload(
  ...
  if (adapter.provider === "r2")
    return adapter.bucket.createMultipartUpload(key, options);
  const headers = {};
  const contentType =
    options.httpMetadata?.contentType || options.contentType || "";
  if (contentType) headers["content-type"] = contentType;
  ...
  const uploadId = decodeXml(
    xml.match(/<UploadId>([\s\S]*?)<\/UploadId>/)?.[1] || "",
  );
...
export async function storageList(env, storageId, options = {}, extra = {}) {
  const adapter = await getStorageAdapter(env, storageId);
  if (adapter.provider === "r2")
    return listR2Objects(adapter.bucket, options, extra);
  ...
  params.set(
    "max-keys",
    String(Math.min(Number(options.limit || extra.maxObjects || 1000), 1000)),
  );
  const res = await signedS3Request(adapter.space, "GET", "", {
    query: `?${params.toString()}`,
  });
  const xml = await res.text();
  const stripPrefix = (value) =>
    value.startsWith(adapter.space.prefix || "")
      ? value.slice((adapter.space.prefix || "").length)
      : value;
  const objects = [
    ...xml.matchAll(
    ...
    key: stripPrefix(decodeXml(match[1])),
    uploaded: new Date(match[2]),
    size: Number(match[3]),
  }));
  ...
  return {
    ...
    delimitedPrefixes,
    truncated,
    cursor: token ? decodeXml(token) : undefined,
  };
}
...
export async function testS3StorageSpace(space) {
  ...
  if (!normalized)
    return { success: false, message: "Invalid S3 space config" };
  const missing = [
    "endpoint",
    "bucket",
    "accessKeyId",
  ...
  try {
    ...
    const res = await signedS3Request(normalized, "GET", "", {
      query: `?${params.toString()}`,
    });
...
export async function handleAdminStorage(env, request, method) {
  ...
  if (method === "PUT") {
    ...
    const currentById = new Map(
      (current.spaces || []).map((item) => [item.id, item]),
    );
    const nextSpaces = (Array.isArray(body.spaces) ? body.spaces : []).map(
      (item) => {
    ...
    await addLog(
      ...
      request,
      "STORAGE_SETTINGS",
      `保存存储空间配置，S3 ${saved.spaces.length} 个，绑定 ${saved.bindings.length} 条`,
    );
    return jsonResponse({
      success: true,
...
export async function handleAdminStorageTest(env, request) {
  ...
  await addLog(
    ...
    request,
    "STORAGE_TEST",
    `${result.success ? "成功" : "失败"}：${space?.name || space?.id || "S3"}`,
  );
