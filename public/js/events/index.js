export function registerAppEvents(deps) {
  ...
  documentRef.addEventListener(
    ...
    (event) => {
      ...
      const filterPopup = documentRef.querySelector(
        '[data-role="kind-filter-popup"]',
      );
      ...
      const notifWrap = documentRef.querySelector(
        '[data-component="notifications"]',
      );
      if (
        notifWrap &&
        !notifWrap.contains(event.target) &&
        store.getState().admin.notifOpen
      ) {
      ...
      if (actionNode) {
        ...
        if (action === "confirm-upload") {
          ...
          if (modal && modal.type === "upload-confirm" && modal.files) {
            store.dispatch(
              actions.uploads.setConflictMode(modal.conflictMode || "rename"),
            );
        ...
        if (action === "toggle-theme") {
          ...
          if (!stored) {
            ...
            const prefersDark = windowRef.matchMedia(
        ...
        if (action === "execute-delete-storage-space") {
          ...
          const config = store.getState().admin.storageConfig;
          if (!config) return;
          const updatedSpaces = (config.spaces || []).filter(
            (s) => s.id !== modal.id,
          );
          const updatedBindings = (config.bindings || []).filter(
            (b) => b.storageId !== modal.id,
          );
          store.dispatch(actions.app.setModal(null));
          store.dispatch(
            thunks.saveAdminStorageConfig({
              ...config,
        ...
        if (action === "toggle-search-filters") {
          store.dispatch(
            actions.explorer.setShowFilters(
              !store.getState().explorer.showFilters,
            ),
          );
          return;
        }
        ...
        if (action === "execute-maintenance-action") {
          ...
          store.dispatch(
            thunks.executeMaintenanceAction(modal.maintenanceAction),
          );
          return;
        }
        ...
        if (action === "execute-delete-webhook") {
          const modal = store.getState().app.modal;
          if (!modal) return;
          const webhooks = (store.getState().admin.webhooks || []).filter(
            (w) => w.id !== modal.id,
          );
          store.dispatch(actions.app.setModal(null));
        ...
        if (action === "copy-share-link") {
          if (!key) return;
          copyText(
            `${windowRef.location.origin}/share.html?token=${encodeURIComponent(key)}`,
            "分享链接已复制",
          );
          return;
        }
        ...
        if (action === "cycle-sort") {
          const next =
            state.explorer.sort === "smart"
              ? "time"
              : state.explorer.sort === "time"
                ? "size"
                : "smart";
        ...
        if (action === "toggle-view") {
          store.dispatch(
            actions.explorer.setView(
              state.explorer.view === "grid" ? "list" : "grid",
            ),
          );
          return;
        }
        ...
        if (action === "toggle-trash") {
          ...
          store.dispatch(
            actions.explorer.setQueryDraft(
              next ? state.explorer.queryDraft : "",
            ),
          );
          store.dispatch(
          ...
          store.dispatch(
            actions.explorer.setClipboard(
              next ? null : state.explorer.clipboard,
            ),
          );
          store.dispatch(thunks.loadExplorer());
          return;
        ...
        if (action === "toggle-filter-popup") {
          const popup = documentRef.querySelector(
            '[data-role="kind-filter-popup"]',
          );
          if (popup) {
        ...
        if (action === "toggle-pick") {
          ...
          if (state.explorer.trashMode) {
            ...
            store.dispatch(
              actions.explorer.setTrashSelectedKeys([...selected]),
            );
          } else {
        ...
        if (action === "toggle-all-pick") {
          ...
          store.dispatch(
            actions.explorer.setSelectedKeys(allSelected ? [] : allKeys),
          );
          return;
        }
        ...
        if (action === "copy-selected" || action === "move-selected") {
          ...
          store.dispatch(
            actions.explorer.setClipboard({
              action: action === "move-selected" ? "move" : "copy",
              paths,
            }),
          );
          store.dispatch(actions.explorer.setSelectedKeys([]));
          dispatchToast(
            "success",
            action === "move-selected" ? "已加入移动队列" : "已加入复制队列",
          );
          return;
        }
        ...
        if (action === "open-entry") {
          ...
          if ((entry.kind || inferKind(entry)) === "folder") {
            if (requiresProtectedUnlock(entry)) {
              openProtectedUnlockModal(
                getEntryPath(entry),
                createDeferredAction("navigate", { path: getEntryPath(entry) }),
              );
              return;
            }
        ...
        if (action === "download-entry") {
          ...
          if (entry) {
            if (requiresProtectedUnlock(entry)) {
              openProtectedUnlockModal(
                getEntryPath(entry),
                createDeferredAction("download", { path: getEntryPath(entry) }),
              );
              return;
            }
            openDownload(entry);
        ...
        if (action === "copy-direct-link") {
          ...
          if (!entry) return;
          copyText(
            `${windowRef.location.origin}/api/preview/${encodeRouteKey(getEntryPath(entry))}`,
            "直链已复制",
          );
          return;
        }
        ...
        if (action === "toggle-markdown-raw") {
          ...
          if (!modal || modal.type !== "preview") return;
          store.dispatch(
            actions.app.setModal({ ...modal, showRaw: !modal.showRaw }),
          );
          return;
        }
        ...
        if (action === "execute-clear-trash") {
          ...
          if (modal && modal.type === "confirm-clear-trash") {
            store.dispatch(
              actions.app.setModal({ ...modal, loading: true, error: "" }),
            );
          }
  ...
  documentRef.addEventListener(
    ...
    (event) => {
      ...
      if (actionInput === "set-logs-filter") {
        ...
        store.dispatch(
          actions.admin.setLogsFilter({ [key]: event.target.value }),
        );
        return;
      }
  ...
  documentRef.addEventListener(
    ...
    (event) => {
      ...
      if (action === "set-upload-conflict-mode") {
        ...
        if (modal && modal.type === "upload-confirm") {
          store.dispatch(
            actions.app.setModal({
              ...modal,
              conflictMode: event.target.value,
            }),
          );
        }
        return;
      ...
      if (role === "filter-kind") {
        ...
        if (store.getState().explorer.query.trim())
          store.dispatch(thunks.loadExplorer());
        return;
      }
  ...
  documentRef.addEventListener(
    ...
    (event) => {
      ...
      if (form === "folder") {
        store.dispatch(
          thunks.createFolder(String(data.get("folderName") || "")),
        );
        return;
      }
      ...
      if (form === "rename") {
        ...
        store.dispatch(
          thunks.renameEntry(path, String(data.get("newName") || "").trim()),
        );
        return;
      }
      ...
      if (form === "unlock-path") {
        store.dispatch(
          thunks.unlockProtectedPath(String(data.get("password") || "")),
        );
        return;
      }
      ...
      if (form === "add-protected-path") {
        store.dispatch(
          thunks.createAdminProtectedPath(
      ...
      if (form === "add-hidden-path") {
        store.dispatch(
          thunks.createAdminHiddenPath(String(data.get("path") || "").trim()),
        );
        return;
      }
      ...
      if (form === "edit-storage-quota") {
        const config = store.getState().admin.storageConfig;
        if (!config) return;
        const r2QuotaBytes = parseInt(
          String(data.get("r2QuotaBytes") || "0"),
          10,
        );
        store.dispatch(
          thunks.saveAdminStorageConfig({
            ...config,
            r2QuotaBytes: isNaN(r2QuotaBytes) ? 0 : r2QuotaBytes,
          }),
        );
        return;
      }
      ...
      if (form === "add-storage-space") {
        ...
        store.dispatch(
          thunks.saveAdminStorageConfig({ ...config, spaces: updatedSpaces }),
        );
        return;
      }
      ...
      if (form === "add-webhook" || form === "edit-webhook") {
        ...
        const events = String(data.get("events") || "")
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean);
        const webhook = {
        ...
        const updated = isEdit
          ? existing.map((w) => (w.id === webhook.id ? webhook : w))
          : [...existing, webhook];
