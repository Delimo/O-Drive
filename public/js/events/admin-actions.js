...
export function registerAdminActions(documentRef, windowRef, store, actions, thunks, dispatchToast, copyText) {
  const handle = (event) => {
    ...
    if (action === "show-add-webhook") {
      store.dispatch(actions.app.setModal({ type: "add-webhook", loading: false, error: "", name: "", url: "", msgtype: "json", method: "POST", contentType: "application/json", headers: "", body: "", events: [], eventMode: "all", enabled: true }));
      return;
    }
