export function createApiLayer(deps) {
  ...
  const fileApi = {
    ...
    createFolder(parentPath, folderName, storageId) {
      const route = parentPath
        ? `/api/mkdir/${encodeRouteKey(parentPath)}`
        : "/api/mkdir";
      return request(route, {
        method: "POST",
    ...
    upload(targetDir, file, targetName) {
      const route = targetDir
        ? `/api/files/${encodeRouteKey(targetDir)}?conflict=rename`
    ...
    uploadWithProgress(
      ...
      const route = targetDir
        ? `/api/files/${encodeRouteKey(targetDir)}?conflict=${conflict}`
        : `/api/files?conflict=${conflict}`;
      const form = new FormDataImpl();
      form.append("file", file, targetName);
      ...
      return new Promise((resolve) => {
        ...
        xhr.onerror = () => {
          resolve({
            response: { ok: false, status: xhr.status || 0 },
            data: null,
          });
        };
  ...
  const adminApi = {
    ...
    deleteProtectedPath(path) {
      return request(
        `/api/admin/settings/protected?path=${encodeURIComponent(path)}`,
        {
          method: "DELETE",
          csrf: true,
        },
    ...
    deleteHiddenPath(path) {
      return request(
        `/api/admin/settings/hidden?path=${encodeURIComponent(path)}`,
        {
          method: "DELETE",
          csrf: true,
        },
  ...
  const multipartApi = {
    ...
    uploadPart(key, uploadId, partNumber, blob, storageId, onProgress) {
      ...
      return multipartApi._xhrUpload(
        `/api/upload-multipart/part?${params.toString()}`,
        blob,
        onProgress,
      );
    },
