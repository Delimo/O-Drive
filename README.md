...
- 管理后台提供概览、操作日志、环境检查、维护工具、存储配额和 Webhook 通知。
- Webhook 支持自定义 URL、method、content_type、headers、body、Basic Auth 和消息格式。
- 移动端可浏览、预览和执行管理员批量操作。
...
| `WEBHOOK_URLS` | 否 | `https://example.com/webhook` | 兼容旧配置，多个 URL 用英文逗号分隔；推荐改用后台 Webhook 页面配置 |
...
7. 打开 `/admin.html`，检查概览、环境检查、维护工具和日志。
8. 在维护工具中执行“重建文件索引”，确认索引可同步。
...
“操作日志”记录管理员上传、删除、移动、复制、重命名、维护等关键操作，便于追踪变更来源。
...
打开 `/admin.html`，进入“维护工具”，执行“重建文件索引”。
