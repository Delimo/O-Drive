...
O-Drive 是一个运行在 Cloudflare Pages 上的轻量云盘。它用 R2 保存文件，用 D1 保存配置、日志、回收站、分享链接、访问控制、文件索引、存储配额和 Webhook 记录。
...
文本类文件可以在线编辑并保存。保存后会写回 R2，并同步文件索引。
