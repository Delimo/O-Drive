...
| `DAV_TOKEN` | 否 | `生成的随机字符串` | WebDAV 访问令牌，留空则禁用 WebDAV |
...
2. 在 Cloudflare Pages 的环境变量中添加：

| 变量名 | 值 |
| --- | --- |
| `DAV_TOKEN` | 上一步生成的随机字符串 |
...
```bash
rclone config create odrive webdav url https://your-domain/dav/ user admin pass your-dav-token
rclone ls odrive:
```
...
将 `DAV_TOKEN` 环境变量留空或删除，然后重新部署。未配置 `DAV_TOKEN` 时，`/dav/*` 路径返回 404。
