...
- 小文件普通上传，大文件自动分片上传，支持选择文件夹并保留目录结构
- 图片缩略图，以及图片、视频、音频、PDF、文本、Markdown 预览
- 文本文件在线编辑保存
- 搜索、筛选、排序、详情面板
- 批量选择和批量操作
- 回收站恢复、彻底删除、清空、筛选、按保留天数清理
- 隐藏路径管理
- 路径访问密码管理，支持错误次数限制
- 管理控制台、操作日志、存储概览、环境检查和维护工具
- 移动端浏览和管理员批量操作
...
| 类型 | 变量名 | 说明 |
| --- | --- | --- |
| D1 database | `D1` | 保存日志、设置、回收站、访问密码和文件索引 |
| R2 bucket | `R2` | 保存实际文件内容 |
...
如果页面返回 500，优先检查 `D1` 和 `R2` 绑定是否存在。如果登录失败，检查环境变量是否部署到了当前 Pages 环境。如果管理员操作提示安全校验过期，刷新页面后重新登录。

## 管理建议

- 大目录删除、移动、复制前先看确认框中的路径和对象数量，超大目录建议分批处理。
- 回收站会继续占用 R2 空间，建议设置保留天数并定期清理。
- 文件很多时建议使用管理控制台维护工具里的“重建文件索引”，让搜索和统计优先使用 D1 索引。
- 回收站项目很多时，可以按路径、类型和删除日期筛选后再恢复或彻底删除。
