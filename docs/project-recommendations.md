...
- 新增 `public/js/render/components.js`，提供通用空状态、详情行和表单反馈渲染 helper。
- `public/js/render/shared.js` 已复用通用详情行和空状态 helper。
- `public/js/render/pages/admin/components.js` 已复用通用空状态 helper。
- 已补充公共 UI helper 渲染测试。
...
- 新增 `public/js/state/thunks/errors.js`，提供通用 `assertApiOk()` 响应断言 helper。
- 分享相关 thunk 已改用 `assertApiOk()`，减少重复的 `response.ok` / `success` 判断。
- 文件夹统计、保存文本、重命名、删除、粘贴等 explorer thunk 已接入 `assertApiOk()`。
- 已保留批量删除/粘贴的部分完成 `completed` 兼容语义。
- 已补充 API 断言 helper 测试。
