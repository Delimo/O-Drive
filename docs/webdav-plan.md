...
2. 服务端解码得到 `username` 和 `password`
3. 验证 `username === env.ADMIN_USERNAME`
4. 验证 `password === env.ADMIN_PASSWORD`（timing-safe 比较）
...
当 `ADMIN_USERNAME` 或 `ADMIN_PASSWORD` 未配置时，WebDAV 功能自动禁用。
...
当 `ADMIN_USERNAME` 或 `ADMIN_PASSWORD` 未配置时，WebDAV 功能自动禁用（返回 404）。
...
- ADMIN_USERNAME 或 ADMIN_PASSWORD 未配置时返回 404
