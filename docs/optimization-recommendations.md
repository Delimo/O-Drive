...
- 系统 Tab 的后台任务列表已补充 ZIP 任务诊断：中文任务类型/状态、失败原因、产物路径、文件名、结束时间和可重试提示。
- ZIP 完成任务继续展示下载结果链接；失败或部分失败任务继续走 `data-action="retry-task"` 重试。
- 已补充前端回归测试，覆盖 ZIP 任务下载链接、失败诊断和重试入口。
...
- 上传服务层已暴露本地 multipart 断点检查，队列入列时会标记 `resumable`、已完成分片数和总分片数。
- 分片上传从本地记录恢复时会立即回传恢复进度，上传面板展示“断点 x/y”和诊断文字。
- 失败/取消后的操作文案改为“重新选择文件”，避免误导用户以为浏览器仍保留原始 File 句柄；重新选择同一大文件时会继续复用本地断点。
- 已补充上传面板回归测试，覆盖断点标签、失败诊断和重新选择入口。
...
当前 `docs/` 目录没有发现已完成的一次性 mockup、临时交接稿或过期路线图；现有 8 个文档均保留为长期维护入口或长期优化记录。

已更新 `docs/README.md` 作为文档治理索引，明确长期保留清单、临时文档清理规则，以及 `frontend-style.md`、`layout.md` 仍作为设计/布局参考保留。
