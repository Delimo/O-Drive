...
> `settings.js` 只做透传转发（创建 → 导出），**不需要改**。⚠️ 注意：`index.js` **没有导入** `settings.js`，而是直接导入 5 个渲染器。`settings.js` 是一个独立的 facade，其导出的方法（如 `renderAdminHealthSection`、`renderAdminQuotaSection`）可能被其他模块使用，但不参与 tab 路由。
...
- 筛选下拉 ★ `data-action-change="set-shares-filter"`（通过 `components.renderCustomSelect` 渲染，非原生 `<select>`）
