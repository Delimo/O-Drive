...
> 本文档用于交接给设计师/前端开发者完成 7 个 tab 页面的 UI 改造。
...
5. [CSS 体系](#5-css-体系)
6. [安全函数](#6-安全函数)
7. [7 个 tab 的数据形状与必留交互](#7-7-个-tab-的数据形状与必留交互)
...
index.js → renderAdminPage()
  ...
  └── <div class="explorer-card">             ← ★ 固定的滚动容器，不要动！
        └── renderAdminActiveTab()            ← 根据 activeTab 分发
              ...
              └── "webdav"    → webdav.renderWebdavSection(admin)
```
...
public/js/render/pages/index.js       ← tab 路由 + .explorer-card 外层容器（不动）
  ...
  ├── admin/webdav.js                 ← WebDAV 管理（独立 tab）
  ├── admin/components.js             ← 共享 UI 组件工厂
  ├── admin/utils.js                  ← 工具函数工厂（createShareUtils）
  ├── admin/paths.js                  ← 被 storage.js 引用（非独立 tab）
  ├── admin/maintenance.js            ← 被 system.js 引用（非独立 tab）
  └── admin/notifications.js          ← 被 system.js 引用（非独立 tab）
```
...
- `public/js/render/pages/admin/webdav.js` — WebDAV 管理

### 存在但非独立 tab 的文件

以下文件存在且功能正常，但不在 `ADMIN_TABS` 路由中。它们被其他渲染器引用：

- `public/js/render/pages/admin/paths.js` — 路径管理渲染器（被 storage.js 引用）
- `public/js/render/pages/admin/maintenance.js` — 运维诊断渲染器（被 system.js 引用）
- `public/js/render/pages/admin/notifications.js` — 通知历史渲染器（被 system.js 引用）
...
- **`public/css/pages/admin.css`** — 所有 admin 样式都在这里，包含两套命名体系：
  ...
  - `.ov-*` 前缀：各 tab 专用样式（`.ov-overview`, `.ov-storage`, `.ov-shares`, `.ov-logs`, `.ov-system`, `.ov-webhook`, `.ov-webdav` 等），被 7 个核心 tab 使用
...
| `public/js/render/pages/admin/paths.js` | 非独立 tab，被 storage.js 引用，按需改 |
| `public/js/render/pages/admin/maintenance.js` | 非独立 tab，被 system.js 引用，按需改 |
| `public/js/render/pages/admin/notifications.js` | 非独立 tab，被 system.js 引用，按需改 |
...
### 7.7 webdav.js — WebDAV 管理

**渲染函数：** `renderWebdavSection(admin)`

此 tab 显示 WebDAV 状态、连接信息和使用说明。

**deps：** `safeText, escapeHtml, renderEmptyStateCompact, components`
...
**必留交互元素：**
- 刷新按钮 `data-action="refresh-admin-health"`
- 复制 URL 按钮 `data-action="copy-webdav-url"` + `data-url`

**状态处理（在渲染函数内部）：** 根据 `health.env.davEnabled` 显示已启用/未配置状态
...
- [ ] `webdav.js` 是独立 tab（第 7 个），依赖 `admin.health.env.davEnabled` 状态
