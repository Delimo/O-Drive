...
> 本文档用于交接给设计师/前端开发者完成 5 个 tab 页面的 UI 改造。
...
5. [CSS 体系](#5-css-体系)
6. [安全函数](#6-安全函数)
7. [5 个 tab 的数据形状与必留交互](#7-5-个-tab-的数据形状与必留交互)
...
后台管理页有 **5 个 tab**，每个 tab 的内容渲染在独立的文件中。
...
public/js/render/pages/index.js       ← tab 路由 + .explorer-card 外层容器（不动）
  ...
  ├── admin/shares.js                 ← 分享
  ├── admin/logs.js                   ← 日志
  ├── admin/system.js                 ← 系统（健康 + 通知 + 运维 + 任务 + Webhook）
  ├── admin/settings.js               ← ⚡ 组合层（facade），透传子渲染器的方法（不直接参与 tab 路由）
  ...
  ├── admin/webhooks.js               ← 存在但当前未被引用（死代码）
...
> `settings.js` 只做透传转发（创建 → 导出），**不需要改**。它不直接参与 tab 路由，但其导出的方法可能被其他模块使用。
...
`.explorer-card` 在 `index.js:152` 定义，是统一的滚动容器：
...
**不要修改** `index.js` 中的 `.explorer-card` 结构和 class，也不要修改它的样式。5 个 tab 的内容都注入在这个容器内部，你只需要修改容器内部的 HTML。
...
- `public/js/render/pages/admin/system.js` — 系统（最复杂）

### 存在但非独立 tab 的文件

以下文件存在且功能正常，但不在 `ADMIN_TABS` 路由中。它们通过 `settings.js` 被间接引用：

- `public/js/render/pages/admin/paths.js` — 路径管理渲染器（被 storage.js 中的路径表格替代）
- `public/js/render/pages/admin/maintenance.js` — 运维诊断渲染器（功能已被 system.js 内联）
- `public/js/render/pages/admin/notifications.js` — 通知历史渲染器（功能已被 system.js 内联）

### CSS 文件

现有 CSS 全部在**一个文件**中（2561 行）：
...
- **`public/css/pages/admin.css`** — 所有 admin 样式都在这里，包含两套命名体系：
  ...
  - `.ov-*` 前缀：各 tab 专用样式（`.ov-overview`, `.ov-storage`, `.ov-shares`, `.ov-logs`, `.ov-system` 等），被 5 个核心 tab 使用
...
| `public/js/render/pages/admin/webhooks.js` | 当前未被引用的死代码，不要改 |
...
**当前 `data-action2` 映射表（在 `events/index.js:74-77` 中）：**
...
> ⚠️ `refresh-admin-quota` 作为 `data-action2` 使用时**未被注册**（事件映射表中没有它）。如果需要同时刷新配额，需要在 `events/index.js` 的 action2 映射中添加。
...
- 新增 CSS class 写在 `public/css/pages/admin.css`
...
⚠️ 注意是 `stats` 不是 `admin`，接收的是 `admin.stats` 子集。错误状态由 `index.js:93` 调用 `renderAdminErrorState` 处理。
...
**状态处理：** 在 `index.js:87-99` 由外层处理 loading / error / 无数据 三种情况，渲染函数只接收已有数据的 `admin.stats`。
...
此 tab 合并了健康探针、通知面板、运维指令、后台调度和 Webhook 管理。
...
> **`MAINTENANCE_ACTIONS` 常量：** `system.js` 和 `maintenance.js` 中都定义了 `MAINTENANCE_ACTIONS` 数组，包含所有可用的维护操作配置 `{ action, label, desc, danger }`。
